...
void doSendCmd(void)
{
	...
	send(sockClient,chSendCmdBuf,9+NetLinkCmd.Len,0);
}
...
{	
	...
	switch(cmd)
	{
		...
		case 'e':
			...
			break;
		case 'f':
			fp=fopen("C:\\Users\\user\\Desktop\\Yan Wei\\C++\\GetPosBySDK - 3DR - twobody\\GetPosBySDK\\pid_pos_velt.txt","r+");
			if(NULL==fp)
			{
				printf("No pid file\n");
			}
			...
			break;
		case 'g':
			fp=fopen("C:\\Users\\user\\Desktop\\Yan Wei\\C++\\GetPosBySDK - 3DR - twobody\\GetPosBySDK\\pid_pendulum.txt","r+");
			if(NULL==fp)
			{
				printf("No pid file\n");
			}
