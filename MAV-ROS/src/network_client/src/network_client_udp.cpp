...
unsigned int addrServer_len;
unsigned int addrClient_len;
...
int main(int argc, char **argv)
{
	...
	while(ros::ok())
	{
		...
		{
			int Recv = recvfrom(sockClient,RecvBuf,128,0,(struct sockaddr *)&addrClient,&addrClient_len);
			//ROS_INFO("NET_LISTENING:%d",RecvBuf[0]);
	        if(Recv > 0)
	        {
		...
		if(enNetLinkState == NET_GETPACKSTART)
		{
			...
			{
				net_link_data.Data.chRecBuf[data_i] = RecvBuf[data_i+6];
			}
        ...
        if(enNetLinkState == NET_GETEND)
        {
			...
			else if(net_link_data.MsgID == 0x05) //data
			{
				...
				optitrack_data.health_flag = net_link_data.Data.chRecBuf[64];
				optitrack_data.pendulum_health_flag = net_link_data.Data.chRecBuf[65];

				optitrack_data.filterveltx = MovingAvgFilter(&VeltFilterX,net_link_data.Data.flRecBuf[6]);
				optitrack_data.filtervelty = MovingAvgFilter(&VeltFilterY,net_link_data.Data.flRecBuf[7]);
				optitrack_data.filterveltz = MovingAvgFilter(&VeltFilterZ,net_link_data.Data.flRecBuf[8]);
				...
				sendto(sockServer,SendBuf,128,0,(struct sockaddr *)&addrServer,sizeof(addrServer));
...
void Init_Internet_Client(void)
{
	ROS_INFO("start init UDP for Optitrack");
	bzero(&addrServer,sizeof(addrServer));
	addrServer.sin_addr.s_addr = inet_addr("192.168.2.254");
	addrServer.sin_family=AF_INET;
	addrServer.sin_port=htons(5999);
	sockServer=socket(AF_INET,SOCK_DGRAM,0);
	addrServer_len = sizeof(addrServer);

	bzero(&addrClient,sizeof(addrClient));
	addrClient.sin_addr.s_addr = inet_addr("192.168.2.253");
	addrClient.sin_family=AF_INET;
	addrClient.sin_port=htons(6001);
	sockClient=socket(AF_INET,SOCK_DGRAM,0);
	bind(sockClient,(struct sockaddr *)&addrClient,sizeof(addrClient));
